 	...
 	async event void Slotter.slot(uint32_t slot) { 		
 		...
 		if (slot < cap) { 
 			...
 			if(TOS_NODE_ID!=0 && slot%(TOTALNODES+1) ==1 && flag == 0){
				...
				/*if(new_opt> CURRNODES && (old_opt >= CURRNODES || old_opt == 0)){
				...
				}else if(new_opt > CURRNODES && old_opt<= CURRNODES && old_opt!=0){
					// start multiplictive increase
					increment = 1;

					CURRNODES = increment+CURRNODES;
					if(CURRNODES>new_opt){
						CURRNODES = new_opt;
					}

					fprintf(node_change_log, "start multiplictive increasing!!!!, increment: %d, CURRNODES:%d, new_opt: %d, old_opt: %d\n", increment, CURRNODES, new_opt, old_opt);
